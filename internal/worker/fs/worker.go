...
func (receiver *FsWorker) ProducePayload() ([]byte, error) {
	for {
		...
		case event := <-receiver.updates:
			...
			fmt.Printf("Sending payload %v to peer\n", wPayload)
			b, err := encodePayload(wPayload)
			fmt.Printf("Payload encoded to %v\n", b)
			return b, err
		}
	}
}
...
func (receiver *FsWorker) ConsumePayload(bytes []byte) error {
	fmt.Printf("Payload decoded from %v\n", bytes)
	payload, err := decodePayload(bytes)
	if err != nil {
		return err
	}

	fmt.Printf("Received payload %v from peer\n", payload)
	...
	go func() {
		for {
			...
			case event, ok := <-watcher.Events:
				...
				fmt.Printf("Got Event for file %v", event.Name)
